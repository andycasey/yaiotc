...
class CannonModel:
    ...
    def fit_spectrum(self, flux, ivar, x0=None, frozen=None, tqdm_kwds=None, n_threads=None, prefer="processes"):
        """
        ...
        print("building args")
        print(f"getting initial guesses")
        if x0 is None:
            x0 = _initial_guess(flux, self.theta, self._design_matrix_indices, self.offsets, self.scales)

        frozen_mask = np.zeros((N, L), dtype=bool)
        ...
        args = (self.theta, self._design_matrix_indices, self.s2, self.offsets, self.scales)


        print(f"Creating iterable")
        iterable = tqdm(zip(flux, ivar, x0, frozen_mask), **_tqdm_kwds)
...
def _initial_guess(flux, theta, idx=None, offsets=0, scales=1, clip_sigma=3):
...
def _fit_spectrum(flux, ivar, x0, frozen_mask, theta, idx, s2, offsets=0, scales=1, clip_sigma=3):
    ...
    any_frozen = np.any(frozen_mask)
    x0_normalized = _normalize(x0, offsets, scales)

    def f(_, *labels):
        # un-freeze anything
        if any_frozen:
            full_labels = np.ones(frozen_mask.size)
            full_labels[frozen_mask] = x0_normalized[frozen_mask]
            full_labels[~frozen_mask] = np.array(labels)
        else:
            full_labels = labels

        l = np.atleast_2d(np.hstack([1, full_labels]))
        A = l.T.dot(l)[idx][np.newaxis]
        return (A @ theta)[0]

    sigma = (ivar / (1. + ivar * s2))**-0.5
    L = len(x0)
    p0 = x0_normalized[~frozen_mask]
    ...
    try:
        ...
        p_opt = np.ones(frozen_mask.size)
        p_opt[frozen_mask] = x0[frozen_mask]
        p_opt[~frozen_mask] = _denormalize(p_opt_norm, offsets[~frozen_mask], scales[~frozen_mask])
